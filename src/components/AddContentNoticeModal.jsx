// Consent-style notice shown before "add restaurant/menu" opens — the actual
// create forms only mount after the user acknowledges this (see App.jsx's
// pendingAddAction gate). Shown every time (no dismiss-persistence), same as
// the other one-off modals in this app (SponsorModal, InstallAppModal).

export default function AddContentNoticeModal({ onConfirm, onClose, t, th }) {
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 210, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 24, padding: "24px 22px", width: "100%", maxWidth: 380, boxShadow: "0 24px 60px rgba(15,23,42,0.3)" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 10, marginTop: 20 }}>
          ...
          <button onClick={onConfirm} style={{ border: "none", background: th.primaryBtn, color: "#fff", borderRadius: 14, padding: "13px 10px", fontWeight: 900, fontSize: 14, cursor: "pointer", fontFamily: "inherit" }}>{t("addNoticeConfirm")}</button>
        </div>
      </div>
    </div>
  );
